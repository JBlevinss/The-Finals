        ...
        match style {
            ...
            | &UseStyle::Named {
                ...
                | Self::AlignmentAnchorForDataInst(_) = self
                {
                    ...
                    let prefix = if is_def && !keyword.is_empty() {
                        // FIXME(eddyb) avoid allocating maybe?
                        format!("{keyword} {name_prefix}").into()
                    } else {
                        Cow::Borrowed(name_prefix)
                    };
                    let suffix = match suffix {
                        Suffix::Num(idx) => format!("{idx}"),
                        Suffix::Name(name) => format!("`{name}`"),
                    };
                    vec![
                        (None, prefix),
                        (
                            // FIXME(eddyb) this doesn't look great for numeric indices.
                            Some(pretty::Styles {
                                thickness: Some(0),
                                size: Some(-2),
